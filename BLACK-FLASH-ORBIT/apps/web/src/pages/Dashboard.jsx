...
import { getSharedDashboardStatus } from "../services/dashboardStatus";

const POLL_INTERVAL_MS = 60000;

const dashboardStatusEndpoint = {
  key: "dashboardStatus",
  load: (userId) => getSharedDashboardStatus(userId),
...
export function Dashboard() {
  ...
  const loadTelemetry = useCallback(async () => {
    ...
    const dashboardResults = await Promise.allSettled([
      dashboardStatusEndpoint.load(user?.id),
    ]);
  ...
  useEffect(() => {
    ...
    const schedulePolling = () => {
      ...
      intervalId = window.setInterval(loadTelemetry, POLL_INTERVAL_MS);
    };
