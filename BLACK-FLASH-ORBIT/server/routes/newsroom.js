...
const NEWSROOM_HISTORY_MAX_CACHE_ENTRIES = 500;
const newsroomHistoryCoalescer = createRequestCoalescer({
  maxEntries: NEWSROOM_HISTORY_MAX_CACHE_ENTRIES,
  resource: "newsroom-history",
  ttlMs: NEWSROOM_HISTORY_CACHE_TTL_MS,
});
const newsroomHistoryVersions = new Map();
...
function pruneOwnerHistoryVersions() {
  while (newsroomHistoryVersions.size > NEWSROOM_HISTORY_MAX_CACHE_ENTRIES) {
    newsroomHistoryVersions.delete(newsroomHistoryVersions.keys().next().value);
  }
}

function getOwnerHistoryVersion(ownerId) {
  const key = String(ownerId || "").trim();

  return key ? newsroomHistoryVersions.get(key) || 0 : 0;
}

function invalidateOwnerHistory(ownerId) {
  const key = String(ownerId || "").trim();

  if (!key) return;

  newsroomHistoryVersions.set(key, getOwnerHistoryVersion(key) + 1);
  pruneOwnerHistoryVersions();
}

function getNormalizedHistoryQuery(queryParams = {}) {
...
function getNewsroomHistoryCacheKey({ ownerId, queryParams }) {
  const userKey = String(ownerId || "").trim();

  return [
    "newsroom-history",
    userKey,
    getOwnerHistoryVersion(userKey),
    getNormalizedHistoryQuery(queryParams),
  ].join(":");
}
...
router.get("/history", requireAuth, async (req, res) => {
  try {
    ...
    const result = await newsroomHistoryCoalescer.resolve(
      getNewsroomHistoryCacheKey({
        ownerId: req.userId,
        queryParams,
      }),
      () =>
      ...
      {
        metadata: {
          userFingerprint: getSafeFingerprint(req.userId),
        },
      },
    );
...
router.post("/history", requireAuth, async (req, res) => {
  try {
    ...
    invalidateOwnerHistory(req.userId);
