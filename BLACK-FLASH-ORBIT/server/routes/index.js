...
const DASHBOARD_PROVIDER_CACHE_TTL_MS = 3_000;

const profileRequestCoalescer = createRequestCoalescer({
  resource: "profile",
  ttlMs: PROFILE_CACHE_TTL_MS,
  maxEntries: 500,
});

const dashboardProviderCoalescer = createRequestCoalescer({
  resource: "dashboard-provider",
  ttlMs: DASHBOARD_PROVIDER_CACHE_TTL_MS,
  maxEntries: 500,
});
...
function mapProject(row) {
  ...
  const lastScan = row?.last_scan || null;
  ...
  return {
    ...
    lastScan,
    telemetrySource: lastScan ? "stored_measurement" : "stored_metadata",
  };
}
...
function mapActivity(row) {
  return {
    ...
    telemetrySource: "supabase_record",
  };
}
...
async function getProjects({ strict = false } = {}) {
  if (!supabase) {
    if (strict) {
      const error = new Error("Supabase dashboard project provider is not configured.");
      error.code = "DASHBOARD_PROJECTS_NOT_CONFIGURED";
      throw error;
    }
  ...
  if (error) {
    ...
    if (strict) {
      const providerError = new Error("Dashboard project provider unavailable.");
      providerError.code = "DASHBOARD_PROJECTS_UNAVAILABLE";
      throw providerError;
    }
...
async function getReports({ strict = false } = {}) {
  if (!supabase) {
    if (strict) {
      const error = new Error("Supabase dashboard report provider is not configured.");
      error.code = "DASHBOARD_REPORTS_NOT_CONFIGURED";
      throw error;
    }
  ...
  if (error) {
    ...
    if (strict) {
      const providerError = new Error("Dashboard report provider unavailable.");
      providerError.code = "DASHBOARD_REPORTS_UNAVAILABLE";
      throw providerError;
    }
...
async function getActivity(limit = 20, { strict = false } = {}) {
  if (!supabase) {
    if (strict) {
      const error = new Error("Supabase dashboard activity provider is not configured.");
      error.code = "DASHBOARD_ACTIVITY_NOT_CONFIGURED";
      throw error;
    }
  ...
  if (error) {
    ...
    if (strict) {
      const providerError = new Error("Dashboard activity provider unavailable.");
      providerError.code = "DASHBOARD_ACTIVITY_UNAVAILABLE";
      throw providerError;
    }
...
function getCachedProfile(user, { force = false } = {}) {
  const userId = getAuthUserId(user);

  return profileRequestCoalescer.resolve(
    `profile:${userId}`,
    () => getProfile(user),
    {
      force,
      metadata: {
        userFingerprint: getSafeFingerprint(userId),
      },
    },
  );
}
...
async function getDashboardProviderSnapshot(user) {
  const userId = getAuthUserId(user);

  return dashboardProviderCoalescer.resolve(
    `dashboard-provider:${userId}`,
    async () => {
      const [projects, reports, activity, readinessSnapshot] = await Promise.all([
        getProjects({ strict: true }),
        getReports({ strict: true }),
        getActivity(20, { strict: true }),
        getReadinessSnapshot(),
      ]);
    ...
    {
      metadata: {
        userFingerprint: getSafeFingerprint(userId || "anonymous-dashboard"),
      },
    },
  );
}
...
router.get("/dashboard/status", async (req, res) => {
  ...
  try {
    const providerSnapshot = await getDashboardProviderSnapshot(req.user);
    projects = providerSnapshot.projects;
    reports = providerSnapshot.reports;
    activity = providerSnapshot.activity;
    readinessSnapshot = providerSnapshot.readinessSnapshot;
  } catch (error) {
    ...
    // Preserve readiness visibility even if one of the telemetry providers fails.
    readinessSnapshot = await getReadinessSnapshot().catch(() => null);
  }
  ...
  return res.json({
    ...
    data: {
      ...
      projects,
      provenance: {
        activity: isDegraded ? "fallback" : "supabase",
        projects: isDegraded ? "fallback" : "supabase",
        reports: isDegraded ? "fallback" : "supabase",
        runtime: "process",
      },
      readiness: readinessSnapshot || { status: "degraded", dependencies: {} },
      security: {
        securityScore: null,
        helmet: "PROTECTED",
        cors: "PROTECTED",
...
router.get("/profile", requireAuth, async (req, res) => {
  ...
  const profile = await getCachedProfile(req.user);
  res.json(profile);
});
