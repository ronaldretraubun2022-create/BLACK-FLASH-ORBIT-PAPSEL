      ...
      projectFlow: hasProjects
        ? projects.slice(0, 4).map((project, index) => {
            ...
            const projectSource =
              ...
              "unknown";
            const sourceLabel = hasRecordedSignal
              ? `${projectSource} • recorded ${formatActivityTime(project.lastScan, "timestamp unavailable")}`
              : `${projectSource} • metadata only`;

            return {
              title:
                project?.name || newsroomFlow[index]?.title || "ORBIT Module",
              body: `${project?.type || "workspace"} • ${status} • ${sourceLabel}`,
